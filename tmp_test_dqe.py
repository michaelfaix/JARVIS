...
def _duration(z=0.0, flag=False):
    """Create a RegimeDurationResult with sensible defaults."""
...
def _fragility(index=0.0):
    """Create a SignalFragilityResult with all scores at the given index."""
...
def _snapshot(records=None):
    """Create a DecisionContextSnapshot."""
    if records is None:
        records = ()
...
def _calm_compute(**overrides):
    """Compute with all-calm defaults (high quality expected)."""
    defaults = dict(
...
def _stressed_compute(**overrides):
    """Compute with all-stressed defaults (low quality expected)."""
    defaults = dict(
...
class TestDurationStressScore:
    ...
    def test_flag_clipped_at_1(self):
        """z=5.0 => 1.0, flag => 1.0 + 0.30 => clipped to 1.0"""
...
class TestStreakInstability:
    ...
    def test_alternating_outcomes(self):
        """WIN, LOSS, WIN, LOSS => 3 transitions / 3 = 1.0"""
    ...
    def test_partial_transitions(self):
        """WIN, WIN, LOSS, LOSS => 1 transition / 3 = 0.333"""
    ...
    def test_window_limited_to_10(self):
        """Only last 10 records considered."""
        # 15 records: first 5 = WIN, next 10 = alternating
        recs = [_rec(i, outcome="WIN") for i in range(5)]
        for i in range(10):
            recs.append(_rec(5 + i, outcome="WIN" if i % 2 == 0 else "LOSS"))
        b = _calm_compute(decision_snapshot=_snapshot(recs))
        # Last 10: alternating => 9 transitions / 9 = 1.0
        assert b.streak_instability == pytest.approx(1.0)
...
class TestRegimeMisalignment:
    ...
    def test_all_different_from_current(self):
        """Last record regime = CRISIS, rest = RISK_ON.
        Current = CRISIS (inferred from last). 4 out of 5 misaligned."""
    ...
    def test_half_misaligned(self):
        """3 RISK_ON + 3 CRISIS (last=CRISIS) => 3/6 = 0.5"""
...
class TestRepeatedFailurePenalty:
    ...
    def test_loss_rate_at_threshold(self):
        """6 out of 10 = 60% => no penalty (threshold is <=)."""
    ...
    def test_loss_rate_above_threshold(self):
        """7 out of 10 = 70% => (0.70 - 0.60)/(1.0 - 0.60) = 0.25"""
...
class TestCompositeQualityScore:
    def test_all_calm_high_quality(self):
        """All sub-scores favorable => high composite."""
        b = _calm_compute()
        # stability=0.9, all others at 0 => quality close to 1.0
        assert b.composite_quality_score > 0.8

    def test_all_stressed_low_quality(self):
        """All sub-scores unfavorable => low composite (floor applied)."""
        b = _stressed_compute()
        # Should be near floor due to cap
    ...
    def test_hand_computed_all_zero_stress(self):
        """All stress at 0, stability at 1.0 =>
        raw = 0.20*1 + 0.15*1 + 0.20*1 + 0.15*1 + 0.10*1 + 0.10*1 + 0.05*1 + 0.03*1 + 0.02*1 = 1.0"""
        b = _calm_compute(
    ...
    def test_hand_computed_all_max_stress(self):
        """All stress at 1.0, stability at 0 =>
        raw = 0.20*0 + 0.15*0 + 0.20*0 + 0.15*0 + 0.10*0 + 0.10*0 + 0.05*0 + 0.03*0 + 0.02*0 = 0.0
        => floor = 0.05"""
        b = _calm_compute(
        ...
        assert b.composite_quality_score == pytest.approx(QUALITY_SCORE_MIN_FLOOR)
...
class TestUncertaintyCap:
    def test_cap_applied_when_uncertainty_high(self):
        """uncertainty > 0.50 => composite capped at 0.60."""
        b = _calm_compute(total_uncertainty=0.6)
        assert b.composite_quality_score <= QUALITY_SCORE_CAP_UNDER_UNCERTAINTY

    def test_no_cap_when_uncertainty_low(self):
        """uncertainty = 0.1 => no cap, quality can be high."""
        b = _calm_compute(
            regime_transition_diagonal_mean=1.0,
            total_uncertainty=0.1,
        )
    ...
    def test_cap_at_boundary(self):
        """uncertainty = 0.50 exactly => no cap (> not >=)."""
        b = _calm_compute(
            regime_transition_diagonal_mean=1.0,
...
class TestSignalPassesSelectivity:
    def test_high_quality_passes(self):
        """High quality, low threshold => passes."""
        b = _calm_compute()
        assert b.signal_passes_selectivity is True

    def test_low_quality_fails(self):
        """All stressed => low quality, high threshold => fails."""
        b = _stressed_compute()
...
class TestHandComputedComposite:
    def test_stability_only_contribution(self):
        ...
        raw = 0.20*0.5 + 0.15*1 + 0.20*1 + 0.15*1 + 0.10*1 + 0.10*1 + 0.05*1 + 0.03*1 + 0.02*1
            = 0.10 + 0.15 + 0.20 + 0.15 + 0.10 + 0.10 + 0.05 + 0.03 + 0.02 = 0.90"""
        b = _calm_compute(
    ...
    def test_fragility_only_contribution(self):
        """stability=1.0, fragility=0.5, rest=0, uncertainty=0 =>
        raw = 0.20*1 + 0.15*1 + 0.20*0.5 + 0.15*1 + 0.10*1 + 0.10*1 + 0.05*1 + 0.03*1 + 0.02*1
            = 0.20 + 0.15 + 0.10 + 0.15 + 0.10 + 0.10 + 0.05 + 0.03 + 0.02 = 0.90"""
        b = _calm_compute(
            regime_transition_diagonal_mean=1.0,
    ...
    def test_duration_stress_contribution(self):
        """stability=1.0, z=2.5 (stress=0.5, no flag), rest=0, unc=0 =>
        raw = 0.20*1 + 0.15*0.5 + 0.20*1 + 0.15*1 + 0.10*1 + 0.10*1 + 0.05*1 + 0.03*1 + 0.02*1
            = 0.20 + 0.075 + 0.20 + 0.15 + 0.10 + 0.10 + 0.05 + 0.03 + 0.02 = 0.925"""
        b = _calm_compute(
            regime_transition_diagonal_mean=1.0,

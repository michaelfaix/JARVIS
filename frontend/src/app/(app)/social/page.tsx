    ...
    <div className="p-2 sm:p-3 md:p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <HudPanel>
          <div className="p-2.5">
            <div className="flex items-center gap-2 mb-1">
              <Heart className="h-3.5 w-3.5 text-hud-red" />
              <span className="text-[10px] text-muted-foreground font-mono">Following</span>
            </div>
            <div className="text-lg font-bold text-white font-mono">
              {followCount}
              {maxFollows !== Infinity && (
                <span className="text-[10px] text-muted-foreground font-normal">/{maxFollows}</span>
              )}
            </div>
          </div>
        </HudPanel>
        <HudPanel>
          <div className="p-2.5">
            <div className="flex items-center gap-2 mb-1">
              <Copy className="h-3.5 w-3.5 text-hud-cyan" />
              <span className="text-[10px] text-muted-foreground font-mono">Active Copies</span>
            </div>
        ...
        <HudPanel>
          <div className="p-2.5">
            <div className="flex items-center gap-2 mb-1">
              <DollarSign className="h-3.5 w-3.5 text-hud-green" />
              <span className="text-[10px] text-muted-foreground font-mono">Copied P&L</span>
            </div>
            <div className={`text-lg font-bold font-mono ${totalCopiedPnl >= 0 ? "text-hud-green" : "text-hud-red"}`}>
        ...
        <HudPanel>
          <div className="p-2.5">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] text-muted-foreground font-mono">Copied Trades</span>
            </div>
            <div className="text-lg font-bold text-white font-mono">
      ...
      <Tabs defaultValue="following">
        <TabsList>
          <TabsTrigger value="following" className="gap-1 data-[state=active]:text-hud-cyan">
            <Users className="h-3 w-3" /> Following
          </TabsTrigger>
          <TabsTrigger value="feed" className="gap-1 data-[state=active]:text-hud-cyan">
            <Activity className="h-3 w-3" /> Activity Feed
          </TabsTrigger>
          <TabsTrigger value="copy" className="gap-1 data-[state=active]:text-hud-cyan">
            ...
            <HudPanel className="mt-4">
              <div className="pt-8 pb-8 px-4 text-center">
                ...
                <Link href="/leaderboard">
                  <Button variant="outline" size="sm" className="text-xs border-hud-border text-hud-cyan hover:bg-hud-cyan/10">
                    Browse Leaderboard
                  </Button>
                </Link>
              </div>
          ...
          <div className="mt-4 space-y-3">
            {followedTraderData.length === 0 ? (
              ...
              followedTraderData.map((trader) => {
                ...
                return (
                  <HudPanel key={trader.id}>
                    <div className="p-2.5">
                      <div className="flex items-start gap-3">
                        {/* Trader Info */}
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-hud-cyan/20 text-hud-cyan text-xs font-bold font-mono">
                        ...
                        <div className="min-w-0 flex-1 space-y-2">
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-medium text-white font-mono">{trader.name}</span>
                              <Badge className={`text-[9px] ${trader.tier === "enterprise" ? "bg-purple-500/20 text-purple-400" : trader.tier === "pro" ? "bg-hud-cyan/20 text-hud-cyan" : "bg-hud-bg/60 text-muted-foreground"}`}>
                                {trader.tier}
                              </Badge>
                              <span className={`text-xs font-mono ${trader.totalReturn >= 0 ? "text-hud-green" : "text-hud-red"}`}>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            ...
                            <div className="flex items-center justify-between sm:flex-col sm:items-start gap-1">
                              <span className="text-[10px] text-muted-foreground font-mono">Copy Trades</span>
                              <Switch
                            ...
                            <div className="space-y-1">
                              <div className="flex items-center justify-between">
                                <span className="text-[10px] text-muted-foreground font-mono">Max Capital</span>
                                <span className="text-[10px] font-mono text-hud-cyan">{settings.maxCapitalPercent}%</span>
                              </div>
                              <input
                                type="range"
                                min={1}
                                ...
                                className="w-full h-1 bg-hud-bg rounded-full appearance-none cursor-pointer accent-hud-cyan"
                            ...
                            {/* Auto-Follow */}
                            <div className="flex items-center justify-between sm:flex-col sm:items-start gap-1">
                              <span className="text-[10px] text-muted-foreground font-mono">Auto-Follow</span>
                              <Switch
                          ...
                          {pnlData && (
                            <div className="border-t border-hud-border/30 pt-2 flex items-center gap-4">
                              <div className="flex items-center gap-1">
                                {pnlData.pnl >= 0 ? (
                                  <TrendingUp className="h-3 w-3 text-hud-green" />
                                ) : (
                                  <TrendingDown className="h-3 w-3 text-hud-red" />
                                )}
                                <span className={`text-xs font-mono font-medium ${pnlData.pnl >= 0 ? "text-hud-green" : "text-hud-red"}`}>
                                  {pnlData.pnl >= 0 ? "+" : ""}${pnlData.pnl.toFixed(2)}
                                </span>
                                <span className="text-[9px] text-muted-foreground font-mono">copied P&L</span>
                              </div>

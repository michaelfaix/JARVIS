...
def map_s05_to_canonical(
    ...
    args = [s05_regime]# type: ignore
    kwargs = {}# type: ignore
    return _mutmut_trampoline(x_map_s05_to_canonical__mutmut_orig, x_map_s05_to_canonical__mutmut_mutants, args, kwargs, None)

def x_map_s05_to_canonical__mutmut_orig(
    s05_regime: str,
) -> Tuple[GlobalRegimeState, AssetRegimeState]:
    """
...
def x_map_s05_to_canonical__mutmut_1(
    s05_regime: str,
) -> Tuple[GlobalRegimeState, AssetRegimeState]:
    """
    Deterministic mapping from removed S05 Regime string to canonical pair.
    Raises KeyError for unknown input — no silent fallback.
    """
    if s05_regime in _S05_TO_CANONICAL:
        raise KeyError(
            f"Unknown S05 regime value: '{s05_regime}'. "
            f"Valid: {list(_S05_TO_CANONICAL.keys())}"
        )
...
def x_map_s05_to_canonical__mutmut_2(
    s05_regime: str,
) -> Tuple[GlobalRegimeState, AssetRegimeState]:
    """
    Deterministic mapping from removed S05 Regime string to canonical pair.
...
def x_map_s05_to_canonical__mutmut_3(
    s05_regime: str,
) -> Tuple[GlobalRegimeState, AssetRegimeState]:
    """
    Deterministic mapping from removed S05 Regime string to canonical pair.
...
x_map_s05_to_canonical__mutmut_mutants : ClassVar[MutantDict] = { # type: ignore
'x_map_s05_to_canonical__mutmut_1': x_map_s05_to_canonical__mutmut_1, 
    'x_map_s05_to_canonical__mutmut_2': x_map_s05_to_canonical__mutmut_2, 
    'x_map_s05_to_canonical__mutmut_3': x_map_s05_to_canonical__mutmut_3
}
x_map_s05_to_canonical__mutmut_orig.__name__ = 'x_map_s05_to_canonical'
...
def map_latent_int_to_canonical(regime_int: int) -> AssetRegimeState:
    args = [regime_int]# type: ignore
    kwargs = {}# type: ignore
    return _mutmut_trampoline(x_map_latent_int_to_canonical__mutmut_orig, x_map_latent_int_to_canonical__mutmut_mutants, args, kwargs, None)

def x_map_latent_int_to_canonical__mutmut_orig(regime_int: int) -> AssetRegimeState:
    """
...
def x_map_latent_int_to_canonical__mutmut_1(regime_int: int) -> AssetRegimeState:
    """
    Maps Kalman LatentState.regime integer (0-4) to AssetRegimeState.
    Internal use only: StateEstimator -> HierarchicalRegimeDetector boundary.
    Raises KeyError for out-of-range values — no silent fallback.
    """
    if regime_int in _LATENT_INT_TO_ASSET_REGIME:
        raise KeyError(
            f"LatentState.regime integer out of range: {regime_int}. Valid: 0-4"
        )
    return _LATENT_INT_TO_ASSET_REGIME[regime_int]

def x_map_latent_int_to_canonical__mutmut_2(regime_int: int) -> AssetRegimeState:
    """
    Maps Kalman LatentState.regime integer (0-4) to AssetRegimeState.
    Internal use only: StateEstimator -> HierarchicalRegimeDetector boundary.
    Raises KeyError for out-of-range values — no silent fallback.
...
x_map_latent_int_to_canonical__mutmut_mutants : ClassVar[MutantDict] = { # type: ignore
'x_map_latent_int_to_canonical__mutmut_1': x_map_latent_int_to_canonical__mutmut_1, 
    'x_map_latent_int_to_canonical__mutmut_2': x_map_latent_int_to_canonical__mutmut_2
}
x_map_latent_int_to_canonical__mutmut_orig.__name__ = 'x_map_latent_int_to_canonical'

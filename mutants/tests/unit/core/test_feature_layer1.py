...
# MUTMUT_UNDER_TEST is only set during mutation runs, not the stats phase,
# so the previous env-var guard did not fire during stats collection.
# Fix: check whether mutmut is installed. In CI/production mutmut is not
# installed -> find_spec returns None -> guard is False -> any real
# "import mutmut" in production feature_layer.py is still caught.
...
class TestNoForbiddenImports:
    ...
    def test_only_stdlib_and_s03_imports_present(self) -> None:
        ...
        for name in self._all_import_module_names():
            ...
            # Skip it when mutmut is installed (dev environment).
